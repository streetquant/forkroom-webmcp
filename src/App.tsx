...
import { createProposal, isDecisionState, reduceDecision } from './domain/reducer'
import type {
  Activity,
  DecisionAction,
  DecisionState,
...
function App() {
  ...
  const importSnapshot = useCallback(async (file: File) => {
    try {
      ...
      if (!isDecisionState(parsed)) throw new TypeError('The file is not a compatible ForkRoom v1 decision snapshot.')
      dispatch({ type: 'load', state: parsed })
      notify(`Imported ${parsed.title}.`, 'success')
    } catch (error) {

...
function optionalInteger(
  ...
  const value = optionalNumber(input, key, minimum, maximum)
...
function serializeResult(tool: string, data: unknown): string {
  return JSON.stringify(
    {
      ok: true,
      tool,
      model_revision_note: 'Results reflect the browser-local workspace at execution time.',
  ...
  return {
    ...
    execute: async (rawInput, { signal }) => {
      ...
      try {
        ...
        return serializeResult(metadata.name, data)
      } catch (error) {
  ...
  const proposeOption = createTool(
    bridge,
    {
      ...
      inputSchema: objectSchema(
        ...
        ['name', 'description', 'thesis', 'rationale'],
      ),
    },
    (input) => {
      rejectUnknownKeys(input, ['name', 'description', 'thesis', 'rationale', 'scores', 'future_impacts', 'assumption_exposures'])
      const state = bridge.getState()
  ...
  const proposeCriterion = createTool(
    bridge,
    {
      ...
      inputSchema: objectSchema(
        ...
        ['name', 'description', 'weight', 'direction', 'unit', 'default_score', 'rationale'],
      ),
    },
    (input) => {
      rejectUnknownKeys(input, ['name', 'description', 'weight', 'direction', 'unit', 'default_score', 'rationale'])
      const name = requiredString(input, 'name', { max: 80 })
      const proposal = bridge.stageProposal({
        kind: 'criterion',
  ...
  const proposeAssumption = createTool(
    bridge,
    {
      ...
      inputSchema: objectSchema(
        ...
        ['statement', 'confidence', 'impact', 'evidence', 'exposures', 'rationale'],
      ),
    },
    (input) => {
      rejectUnknownKeys(input, ['statement', 'confidence', 'impact', 'evidence', 'exposures', 'rationale'])
      const state = bridge.getState()
  ...
  const proposeScenario = createTool(
    bridge,
    {
      ...
      inputSchema: objectSchema(
        ...
        ['name', 'description', 'probability', 'severity', 'impacts', 'rationale'],
      ),
    },
    (input) => {
      rejectUnknownKeys(input, ['name', 'description', 'probability', 'severity', 'impacts', 'rationale'])
      const state = bridge.getState()
  ...
  const proposeScoreChange = createTool(
    bridge,
    {
      ...
      inputSchema: objectSchema(
        ...
        ['option_id', 'criterion_id', 'score', 'evidence', 'rationale'],
      ),
    },
    (input) => {
      rejectUnknownKeys(input, ['option_id', 'criterion_id', 'score', 'evidence', 'rationale'])
      const state = bridge.getState()
  ...
  const challengeAssumption = createTool(
    bridge,
    {
      ...
      inputSchema: objectSchema(
        ...
        ['assumption_id', 'counterpoint', 'test', 'rationale'],
      ),
    },
    (input) => {
      rejectUnknownKeys(input, ['assumption_id', 'counterpoint', 'revised_confidence', 'test', 'rationale'])
      const state = bridge.getState()
  ...
  const draftCommitment = createTool(
    bridge,
    {
      ...
      inputSchema: objectSchema(
        ...
        ['option_id', 'statement', 'guardrails', 'review_date', 'rationale'],
      ),
    },
    (input) => {
      rejectUnknownKeys(input, ['option_id', 'statement', 'guardrails', 'review_date', 'rationale'])
      const state = bridge.getState()
      const id = requiredString(input, 'option_id', { max: 80 })
      const option = requireOption(state, id)
      const proposal = bridge.stageProposal({
